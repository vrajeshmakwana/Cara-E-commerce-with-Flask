from flask import Flask , request, url_for, redirect, render_template, jsonify, session,make_response,json
...
import stripe,secrets,math,logging,requests,pycountry,time
...
# Configuring and Setting JWT Token 
app.config["JWT_SECREY_KEY"] = os.urandom(24)
...
def settings(id):
      name = db.session.query(user_login.Username).filter_by(Token=id).scalar()
      return render_template("settings.html",token=id,name=name)

@app.route("/profile/<string:id>",methods = ["GET"])
def profile(id):
      name = db.session.query(user_login.Username).filter_by(Token=id).scalar()
      return render_template("profile.html",token=id,name=name)
...
def admin_logs():
      with open("activity.log","r",encoding="utf-8") as file:
            ...
            for d in data:
                  logs = d.split("|")
                  print(logs[6][7:].strip() == "Normal")
            

      return render_template("adminLogs.html",data=data,current_page="logs")
...
def edit_product(Sr_No):
      ...
      if "Admin" in session and session["Admin"] == "admin":
            if request.method == "POST":
                  ...
                  req_product_image = request.form.get("pimg")
                  ...
                  product.Product_Image = req_product_image
...
def generate():
      try:
            ...
            tax = subtotal * 0.08
            total = subtotal + tax
            css_path = "E:/Python_Mysql_connect/static/css/pdf.css"
      ...
      if request.method == "POST":
            ...
            if username_input == data.Admin_Username:
                  if bcrypt.checkpw(password_input.encode("UTF-8"), data.Admin_Password.encode("UTF-8")):
                        ...
                        session['ip'] = requests.get("https://api.ipify.org").text
...
def admindashboard():
      if "Admin" in session and  session["Admin"] == "admin" :
            ...
            return render_template("admindashboard.html", products = products_data ,current_page="admin",  name = data.Admin_Username, success=success)
      else:
            print("fail")
      return render_template("form.html")
...
def email_confirmation_message():
      return render_template("emailmessage.html")  
...
def userdashboard(id):
      if "username" in session:
            ...
            return render_template("userdashboard.html", user = user_data ,data = pdata, id=id)
...
def about(id):
      user_data = user_login.query.filter_by(Token = id).first()
      return render_template("about.html", name=user_data.Username, token=id, current_page="about")
...
def shop(id):
      ...
      return render_template("shop.html",name=user_data.Username,products = product_data,prev=prev,next=next, token=id, current_page="shop")

@app.route("/blog/<string:id>", methods=["GET", "POST"])
def blog(id):
      user_data = user_login.query.filter_by(Token = id).first()
      return render_template("blog.html", name=user_data.Username, token=id, current_page="blog")

@app.route("/contact/<string:id>", methods=["GET", "POST"])
def contact(id):
      user_data = user_login.query.filter_by(Token = id).first()
      return render_template("contact.html", name=user_data.Username, token=id, current_page="contact")
...
def product(Token,pid):
      ...
      reviews = product_reviews.query.filter_by(product_id=pid).all()
      name = db.session.query(user_login.Username).filter_by(Token= Token).scalar()
      return render_template("ProductDetails.html",token=Token,name=name,ProductDetails = details,images =images,reviews=reviews ,sizes = final_size,colors=unique_colors,features=unique_features)
...
def show_order(id):
      ...
      try:
            ...
            print(orders_cluster["ORD-4359155241"][0].OrderStatus)
      except Exception as e:
            print("error:-",e)
...
def checkout(id):
      ...
      user = user_login.query.filter_by(Token= id).first()
      product_data = shopping_cart.query.filter_by(user_id = user.Sr_No).all()
      subtotal = sum(subtotal.Product_Price for subtotal in product_data)
      total = sum(total.Total for total in product_data)
      print(subtotal)
      return render_template("checkout.html",product = product_data,subtotal=subtotal, token = id, total=total,user_id = user.Sr_No)
...
def conf():
      if request.method == "POST":
            ...
            try:
                  ...
                  user = user_login.query.filter_by(Token = token).first()
                  data = shopping_cart.query.filter_by(user_id = user.Sr_No).all()
                  order_id = "ORD-" + str(random.randint(1111111111,9999999999))
                  
                  # Inserting shipment data into the database 
                  
                  shipping_data = shipping_details(user_id = user.Sr_No, Name = name,Address = address,
                  ...
                  print(id)
                  for d in data: 
                        order_data = Orders(customer_id = user.Sr_No,customer_name=user.Username, orderID= order_id,Product_Name=d.Product_Name,
                  ...
                  ship_data = shipping_details.query.filter_by(user_id=user.Sr_No).first()
                  ...
                  print(session["ship_data"],session["order_data"])
                  session["token"] = token
                  ...
                  return render_template("checkout.html",url=payment_session.url,) 
            except Exception as e:
                  print("Error:- ",e)
      ...
      if request.method == "POST":
            ...
            #  Step 4:- Editing the product images
             ...
             if main_img.filename == "" and  all(image.filename == "" for image in sub_images):
                  ...
                  product.images[0].url = filepath[23::]
                  ...
                  for index, image in enumerate(sub_images):
                        ...
                        image.save(filepath)
                        product.images[index + 1].url = filepath[23::]
                  ...
                  for index, image in enumerate(image_insert_list):
                        filepath = os.path.join(app.config["UPLOAD_FOLDER"], secure_filename(image.filename))
                        image.save(filepath)
                        product.images[index].url = filepath[23::]
             try:
                   db.session.commit()
...
def productData():
      if request.method == "POST":
            ...
            if main_img:
                  filepath = os.path.join(app.config["UPLOAD_FOLDER"], secure_filename(main_img.filename))
                  main_img.save(filepath)
                  images = product_images(product_id=product_id,url=filepath[23::],is_main=1)
                  db.session.add(images)
                  db.session.commit()
            for image in sub_images:
                  filepath1 = os.path.join(app.config["UPLOAD_FOLDER"] ,secure_filename(image.filename))
                  image.save(filepath1)
                  image = product_images(product_id=product_id,url=filepath1[23::],is_main=0)
    ...
    if result:
       ...
       #  Using try except in order to handle the exceptions
        ...
        try:
            ...
            values = (result.Sr_No,product_id, product_details["image"], product_details["name"], int(product_details["price"]), int(product_details["quantity"]),int(product_details["price"]))
...
def cart(id):
       try:
              print(id)
              user = user_login.query.filter_by(Token = id).first()
              cart_details = shopping_cart.query.filter_by(user_id= user.Sr_No).all()
              print(cart_details)
             ...
             return f"Error:- {e}"
       return render_template("cart.html",token = id, name=user.Username, product_data = cart_details,total = totalPrice)
